                        ...
                        zdt_can_packet_t *out_pkts, int *out_count)
{
     ...
     *   - payload 从 frame[1]（FuncCode）开始，每包填 8 字节
     *   - 长命令（>8 字节 payload，即原 UART 帧 >9 字节）按 Packet 0,1,2... 拆包
     *   - 末包 DLC 可能 < 8
     *
     * 例：UART 帧 "01 36 6B" → CAN ID=0x0100, payload=36 6B (DLC=2)
     *     UART 帧 "01 FD ... 6B"（13B）→ 包0 id=0x0100 payload=FD 01 0F A0 00 00 01 FA (DLC=8)
     *                                 包1 id=0x0101 payload=00 00 00 6B (DLC=4)
     *
     * 真机证据：发 36 6B 电机回复带位置数据；发 01 36 6B 电机回复 EE（格式错误）。
     */
    int idx = 1;            /* frame 中下一个待发送字节，跳过 frame[0]=addr */
    int remaining = frame_len - 1;
    int pkt = 0;
    ...
    if (remaining > 0) {
        /* 超过 ZDT_CAN_MAX_PACKETS 能容纳——理论上 37B 命令最多 6 包，
         * 8 包已留余量；若真触发说明调用方传错。 */
