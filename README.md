...
```
zdt_x42s_can/
├── components/zdt_can/         # 封装组件（核心）
│   ├── include/zdt_can.h
│   ├── zdt_can.c               # TWAI 安装 + 分帧发送 + 接收
│   ├── test/split_host_test.c  # host gcc 自测：3 case 锁定分帧规则
│   └── idf_component.yml       # 声明 git 依赖 libzdt
├── examples/esp32c3_motor_test/  # esp32c3 真机测试工程
...
## host 自测分帧逻辑（不依赖 ESP-IDF）

```bash
gcc -std=c99 -Wall -Wextra components/zdt_can/test/split_host_test.c -o /tmp/t && /tmp/t
# 3 PASS / 0 FAIL —— 锁定分帧规则
```
